...
module.exports.validate = async (sender, recipient, exchange) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        socket.on('next', () => {
            if (i < 3) {
                if (socket.writable) {
                    ...
                    socket.emit('fail', { message: 'SMTP communication unexpectedly closed.', code: 100 });
                }
            } else {
                socket.emit('success');
            }
        ...
        socket.on('timeout', () => {
            socket.emit('fail', { message: 'Timeout', code: 100 });
        });
        ...
        socket.on('connect', () => {
            socket.on('data', msg => {
                ...
                if (hasCode(msg, 220) || hasCode(msg, 250)) {
                    ...
                    if(msg.toLowerCase().includes('spamhaus'))
                        socket.emit('fail', { message: 'IP blocked by Spamhaus', code: 101 });
                    else
                        socket.emit('fail', { message: 'Mailbox not found.', code: 102 });

                } else {
                    const [code] = Object.keys(ErrorCodes).filter(x => hasCode(msg, x))
                    socket.emit('fail', { message: ErrorCodes[code] || 'Unrecognized SMTP response.', code: 100 })
                }
            })
        })
    })
